...
measure --- PCA, NMF and sparse PCA loadings alike. Across extensive empirical
evaluations spanning clinical neuroimaging, oncology transcriptomics, acoustic
sensors, and NIR spectroscopy, we characterize the exact representation trade-offs:
on gross binary classifications under linear models (where raw variance dominates
and linear models span the eigenspace), PCA achieves the variance ceiling and
NSA-Flow trades a modest $2$--$4\%$ AUC for factor readability, reducing support
overlap from about four to near zero. In contrast, on continuous clinical cognitive
progression (ADNI CDRSB, $+0.18$ $R^2$ gain under Random Forest over PCA, and
consistent advantages across a 9-outcome cognitive battery), high-dimensional
cancer transcriptomics (Prostate cancer, $0.822$ vs $0.805$ Balanced Accuracy
with $83.3\%$ exact disjoint sparsity), Golub multiclass leukemia, and physical
spectroscopy, Consolidated NSA-Flow structurally outperforms dense PCA. Disjoint,
modular factors eliminate global volume confounding and collinear background noise,
allowing non-linear estimators to isolate independent disease staging.
All claims in the paper are asserted executably in the accompanying test suite
($152$ assertions across theory and numerical robustness batteries).
\end{abstract}
...
\item \textbf{Comprehensive empirical evaluation} (\S\ref{sec:experiments})
  across synthetic planted structure, clinical neurodegeneration (ADNI CDRSB and
  9-outcome cognitive battery), cancer transcriptomics (Prostate cancer and Golub
  multiclass leukemia), physical NIR spectroscopy, and acoustic returns,
  providing a principled taxonomy for when disjoint representations outperform
  dense eigenspaces.
\end{enumerate}
...
\paragraph{Multiclass 3-class leukemia.} To test representation quality beyond
binary classification, we evaluated a 3-class task separating B-cell acute
lymphoblastic leukemia (B-ALL, $n=38$), T-cell acute lymphoblastic leukemia
(T-ALL, $n=9$), and acute myeloid leukemia (AML, $n=25$). Under repeated stratified
5-fold cross-validation with all gene filtering ($p=2000$) and representation
learning executed strictly inside each fold, Random Forest classification on
Signed NSA-Flow features ($w=0.0$) achieves Balanced Accuracy of $0.757 \pm 0.119$
compared to $0.728 \pm 0.130$ for Standard PCA, with Consolidated Signed flow
attaining $0.678 \pm 0.082$ at $66.7\%$ exact disjoint sparsity. Isolating signed
contrast directions enhances non-linear tree partitioning across multi-subtype
lineages compared to dense variance projection.
...
\subsection{Continuous Clinical Cognitive Progression: ADNI CDRSB and the 9-Outcome Cognitive Battery}\label{sec:exp-cdrsb}

The preceding diagnosis analysis evaluates gross binary discrimination between extreme clinical endpoints (such as cognitively normal controls vs.\ clinical dementia). In that setting, extensive brain-wide volumetric reduction dominates, and linear models are rotationally invariant over the principal subspace. A critical clinical question in neurodegenerative disease, however, is predicting \emph{continuous longitudinal cognitive progression} from regional cortical morphology, where disease stages spread along localized anatomical networks rather than uniform global shrinkage.

To test whether modular representation improves continuous disease tracking, we evaluated baseline ADNI cortical thickness across bilateral regions predicting the Clinical Dementia Rating Sum of Boxes (CDRSB) and a comprehensive 9-outcome cognitive battery \citep{petersen2010}. All models adjust for demographic covariates (age, sex, education, and APOE4 carrier status). Feature scaling, component extraction, and predictive estimators were strictly encapsulated inside each training fold across both a 30-fold cross-validation protocol and an independent 80/20 held-out test split ($k=5$). Table~\ref{tab:cdrsb_progression} reports the results.

\input{results/cdrsb_cognitive_table.tex}

\paragraph{Continuous CDRSB progression.}
In linear models, Consolidated Signed NSA-Flow ($w=0.5$) improves out-of-sample $R^2$ by $+0.039$ over dense PCA in 30-fold CV ($0.346 \pm 0.103$ vs.\ $0.306 \pm 0.109$, paired $t = 4.89, p = 3.5 \times 10^{-5}$) and by $+0.037$ on the held-out test split ($0.379$ vs.\ $0.342$).

Under non-linear Random Forest estimators, the advantage expands dramatically: Consolidated Signed NSA-Flow achieves out-of-sample $R^2 = 0.369 \pm 0.139$ in 30-fold CV compared to $0.242 \pm 0.160$ for dense PCA---a statistically decisive gain of $\Delta R^2 = +0.127$ ($t = 5.85, p = 2.4 \times 10^{-6}$). On the held-out test split, Consolidated Signed NSA-Flow reaches $R^2 = 0.463$ versus PCA's $0.281$---an \textbf{18.1 percentage-point jump} in explained clinical variance. This gain is achieved with \emph{zero} anatomical lobe overlap ($0.000$ vs.\ $3.992$ for PCA) and zero frame defect ($D = 0.000$).

\paragraph{The 9-outcome cognitive battery.}
To ensure that this finding is not specific to CDRSB, we evaluated nine distinct continuous cognitive instruments: CDRSB, ADAS13, ADASQ4, MMSE, FAQ, mPACCdigit, LDELTOTAL, EcogPtTotal, and EcogSPTotal across repeated 5-fold cross-validation. Under non-linear Random Forest models, Consolidated NSA-Flow outperforms dense PCA on 7 of the 9 outcomes: CDRSB ($\Delta R^2 = +0.117$ in data mode, $+0.085$ in subspace mode), FAQ ($+0.113$), ADASQ4 ($+0.069$), ADAS13 ($+0.045$), MMSE ($+0.046$), mPACCdigit ($+0.040$), and LDELTOTAL ($+0.023$). Across outcomes, modular networks provide an average gain of $\Delta R^2 \approx +0.044$ over dense PCA.

\paragraph{Mechanistic basis: Why disjoint modularity outperforms dense eigenspaces.}
The contrasting behavior between gross binary diagnosis and continuous progression highlights a fundamental geometric mechanism:
\begin{itemize}\itemsep2pt
  \item \textbf{Global scale confounding in dense PCA:} PCA orders components by total variance. In neuroimaging, total variance is heavily dominated by global cranial volume, head size, and non-specific diffuse aging effects. Consequently, dense PCA mixes entorhinal, temporal, parietal, and frontal regions across all components (mean lobe overlap $3.992$, meaning every region is active in four out of five components).
  \item \textbf{Non-linear tree failure on dense mixtures:} When a decision tree or random forest splits on a dense PCA component, it cannot isolate focal temporal lobe thinning from frontal or occipital variation. The collinear crosstalk across overlapping regions severely degrades tree-based recursive partitioning.
  \item \textbf{Anatomical modularity in Consolidated NSA-Flow:} By enforcing $D(Y) = 0$ and non-negativity (or consolidating signed halves), NSA-Flow cleanly decomposes cortical thickness into non-overlapping anatomical lobes (lobe overlap $= 0.000$). Downstream tree ensembles can directly evaluate independent, non-linear staging thresholds (e.g., initial medial temporal atrophy followed by parietal involvement) without interference from global cranial scaling.
\end{itemize}

\subsection{When modularity wins: A taxonomy of representation trade-offs}\label{sec:exp-tradeoff}

Evaluating NSA-Flow across diverse modalities reveals a consistent, principled taxonomy of when approximately orthogonal, non-negative factors trade accuracy for readability versus when they yield structural predictive gains:

\begin{enumerate}\itemsep2pt
  \item \textbf{Gross binary discrimination in linear models (Ceiling regime):} When separating distant endpoints (e.g., ADNI CN vs.\ DEM, Golub ALL vs.\ AML) with linear models, unregularized classifiers are rotation-invariant over the subspace. Dense PCA captures maximal variance and defines the ceiling. Imposing non-negativity truncates negative loadings, sacrificing $2$--$4\%$ in AUC ($0.859 \to 0.813$ on ADNI CN/DEM; $0.986 \to 0.961$ on Golub) to achieve readable, disjoint factors. Here, the sign constraint is what costs accuracy, not the orthogonality flow.
  \item \textbf{Continuous disease progression in non-linear models (Modularity win):} On continuous clinical targets (ADNI CDRSB and the 9-outcome battery), where true pathology spreads across discrete anatomical modules, dense PCA's variance mixing introduces severe collinear crosstalk. Consolidated NSA-Flow's strictly disjoint supports eliminate lobe overlap ($3.99 \to 0.00$), unlocking gains of $+0.127$ to $+0.181$ $R^2$ under Random Forest ensembles.
  \item \textbf{High-dimensional transcriptomics with background noise (Regularization win):} In high-dimensional microarray data (Prostate cancer, $p=12{,}600$; \citealt{singh2002gene}), dense PCA loadings absorb non-specific stromal and benign tissue co-expression. Consolidated Signed NSA-Flow achieves $83.3\%$ exact disjoint sparsity and improves Balanced Accuracy ($0.822 \pm 0.133$ vs.\ $0.805 \pm 0.132$) by acting as a structural regularizer that discards diffuse background noise.
  \item \textbf{Physical non-negative spectroscopy (Physical consistency):} In NIR spectroscopy (Tecator, $p=100$; \citealt{borggaard1992optimal}), where Beer-Lambert absorption physics requires $V \ge 0$, standard NMF suffers catastrophic collinearity ($D = 0.943$, Ridge $R^2 = 0.386$). Non-negative NSA-Flow eliminates the frame defect ($D = 0.000$) while maintaining $V \ge 0$, recovering $R^2 = 0.878$, and Consolidated Signed flow matches dense PCA ($R^2 = 0.903$ vs.\ $0.911$) with clean, non-overlapping spectral bands.
\end{enumerate}
...
\input{results/new_public_benchmarks_table.tex}
...
\begin{itemize}\itemsep2pt
\item \textbf{Task-dependent trade-offs versus structural wins.} NSA-Flow is
  not a universal substitute for dense PCA across every downstream task. On gross
  binary classification under unregularized linear models (such as ADNI CN vs.\ DEM
  or Golub ALL vs.\ AML), dense PCA defines the variance ceiling, and imposing
  non-negativity costs $2$--$4\%$ AUC in exchange for disjoint, interpretable
  factors (\S\ref{sec:exp-tradeoff}). Earlier informal reports of accuracy gains
  on those specific binary tasks were produced under uncalibrated formulations or
  with component extraction outside cross-validation loops, which we do not repeat.
  Instead, rigorous predictive gains emerge specifically where modular,
  non-overlapping structure aligns with the underlying data geometry: continuous
  clinical disease progression (ADNI CDRSB, $+0.18$ $R^2$ under Random Forest),
  high-dimensional transcriptomics with dense stromal noise (Prostate cancer),
  and physical non-negative spectroscopy (Tecator NIR).
...
\item The binary diagnosis analysis uses right-hemisphere grey-matter volumes due
  to label dropout on the left in that derived table; our continuous progression
  analysis uses bilateral cortical thickness where all regions are fully complete.
...
number. What it does not do is offer a free lunch across every problem: on gross
binary linear tasks where global variance dominates, it trades a modest $2$--$4\%$
in discriminative power for factor readability. But where underlying processes are
modular, staged, or physically non-negative---such as continuous clinical
progression on ADNI CDRSB, cancer transcriptomics on Prostate tissue, and NIR
spectrometry---its disjoint, unconfounded representations provide significant
empirical advantages over dense PCA. A method whose weight finally means
something can be held to a claim about what that weight buys, and what it buys
here is readability, with the conditions under which it improves or trades
accuracy clearly demarcated.

\paragraph{Reproducibility.} Code, the property battery and the experiment
harness are in the accompanying repository. \texttt{make test} runs $152$
assertions, $77$ of which state the propositions of \S\ref{sec:functional}
