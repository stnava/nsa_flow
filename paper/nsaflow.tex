...
loop is factorisation-free at $O(pk^2)$, converges in tens to a few hundred
deterministic iterations, and---empirically, over random restarts on every
problem family we tried---reaches a unique optimum for $w<1$, so there are no
restarts, schedules or learning-rate heuristics to tune. On synthetic data with a
planted non-negative basis, on Golub leukemia expression, and on ADNI regional
brain volumes, $w$ traces an interpretable path from plain reconstruction to
disjoint factors, and the quantitative bound
$\max_{i\ne j}\ip{y_i}{y_j} \le \sqrt{D}\,\Fnorm{Y}^2$ makes ``approximately
disjoint'' a statement with a number attached. All claims in the paper are
asserted executably in the accompanying test suite.
\end{abstract}
...
optimal. The useful regime is therefore $w<1$, where the fidelity term selects
among clusterings. Together with Remark~\ref{rem:whyrelax} this identifies $w<1$
...
bounded away from zero whenever $w\tilde D(X_0) < 1-w$. Across every experiment
reported here $\Fnorm{Y}/\Fnorm{X_0}$ remained in $[0.55, 0.95]$.
...
\subsection{Recovery of a planted non-negative basis}

We generate $X = ZV_{\text{true}}^\top + \sigma\varepsilon$ with
$V_{\text{true}}$ a non-negative disjoint-support basis ($p=60$, $k=6$, $n=300$),
so the ground truth lies in the $w\to1$ solution class, and measure recovery of
$V_{\text{true}}$ from PCA loadings refined at each $w$. Baselines are PCA,
sparse PCA and NMF. Figure~\ref{fig:recovery} and Table~\ref{tab:recovery} report
mean matched $|\cos|$, support F1, and the mean number of extra components each
active feature loads on.
...
\caption{Recovery of a planted non-negative basis as a function of $w$, at four
noise levels, against PCA, sparse PCA and NMF (grey lines). (c) reports factor
readability: zero means every feature loads on exactly one component.}
\label{fig:recovery}
\end{figure}
...
partition. Table~\ref{tab:clustering} reports the adjusted Rand index of that
partition against the planted truth and against $k$-means run directly on the
features, confirming Proposition~\ref{prop:disjoint} operationally: as $w\to1$
NSA-Flow converges on a clustering, and it is close to the one $k$-means finds.
...
\emph{not} an improvement in classification. Plain PCA is the strongest
predictor (AUC $\approx 0.99$, accuracy $\approx 0.94$); NSA-PCA gives up a
small amount of both. What it buys is factor readability: the mean number of
extra components each active gene loads on falls from about $4$ (PCA: every gene
loads on every component) to below $0.2$ at $w=0.99$, and loading sparsity rises
from essentially zero to over $0.76$. Sparse PCA achieves part of the same
readability gain, at intermediate sparsity and without non-negativity or a
clustering interpretation.

\input{results/e4_table.tex}

\subsection{ADNI regional brain volumes}\subsection{ADNI regional brain volumes}
...
The pattern matches Golub. Across all three tasks PCA attains the highest AUC
and NSA-PCA is a little below it, while support overlap falls from $4.0$ (every
region on every component) to about $0.37$. The one place $w$ helps
monotonically is the hardest task, CN vs MCI, where AUC rises with $w$ across
the whole range --- consistent with the idea that decorrelated, localised
components are less prone to overfitting a weak signal --- but even there it does
not overtake PCA.
...
Read that way, essentially the whole accuracy gap is paid at $w=0$. On ADNI
CN vs DEM, PCA reaches AUC $\approx 0.86$ and $w=0$ drops to $\approx 0.81$;
raising $w$ from $0$ to $0.99$ then moves AUC by a fraction of that while
reducing support overlap by an order of magnitude. On Golub the same holds, with
AUC recovering slightly as $w$ grows before falling again at $w=0.99$. The cost
of interpretability here is largely the cost of the sign constraint, not of the
orthogonality relaxation --- which is the useful thing to know, because the sign
constraint is what makes the factors additive and nameable in the first place.
...
end-to-end time. Table~\ref{tab:svd} documents the SVD-gradient failure of
\S\ref{sec:algorithm}. Table~\ref{tab:lr} records a separate lesson: in v1 the
...
harness are in the accompanying repository. \texttt{make test} runs $133$
