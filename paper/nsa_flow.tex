...
departs from Riemannian gradient descent
(\citeproc{ref-absil2008optimization}{Absil, Mahony, and Sepulchre
2008}) in treating orthogonality as an objective rather than a
constraint, so no retraction is required and no factorisation appears in
the iteration. Non-negativity is the one hard constraint, enforced
exactly by proximal projection (\citeproc{ref-parikh2014proximal}{Parikh
and Boyd 2014}), and descent is guaranteed by a line search rather than
by manifold geometry. Conceptually, NSA-Flow functions as a soft
projection operator that can be inserted into any existing machine
learning system to improve interpretability---whether as a
regularization layer in a neural network
...
where \(w \in [0, 1]\) balances the fidelity loss \(L_{fid}\) against
the orthogonality loss \(L_{orth}\). Both losses are \emph{dimensionless
by construction}, so no internal re-weighting, warm-up calibration or
data-dependent scaling constant is required and \(w\) is a genuine
convex weight. The fidelity term is the relative squared Frobenius
distance \[
L_{fid}(Y, X_0) = \frac{\| Y - X_0 \|_F^2}{\| X_0 \|_F^2},
\] which is \(0\) at \(Y = X_0\) and exactly \(1\) at \(Y = 0\).

For the orthogonality loss, write
\(G = Y^\top Y / \operatorname{tr}(Y^\top Y)\) for the trace-normalised
Gram matrix. We use one of two penalties, and the distinction is not
cosmetic: \[
D(Y) = \Big\| G - \tfrac{1}{k} I_k \Big\|_F^2,
\qquad
C(Y) = \frac{1}{k(k-1)} \sum_{i \neq j} \cos^2(y_i, y_j),
\] with
\(\cos(y_i, y_j) = \langle y_i, y_j\rangle / (\|y_i\|\,\|y_j\|)\). \(D\)
is scale invariant, lies in \([0, 1 - 1/k]\), and vanishes exactly on
the scaled Stiefel manifold \(\mathbb{R}_{>0}\cdot St(p,k)\); we report
\(\tilde D = D / (1 - 1/k)\) so that it lies in \([0,1]\) and equals
\(1\) at rank one. It admits the spectral reading \[
D = k \operatorname{Var}(\lambda_i(G)) = \frac{1}{\mathrm{EffRank}(Y)} - \frac{1}{k},
\] so it is a participation-ratio deficit, and \(D \ge 1/r - 1/k\)
whenever \(\operatorname{rank}(Y) = r < k\): rank collapse is penalised
rather than rewarded.

Crucially, \(D\) decomposes as \[
D = \underbrace{\sum_{i \neq j} G_{ij}^2}_{\text{decorrelation}}
  + \underbrace{\sum_i \big(G_{ii} - \tfrac1k\big)^2}_{\text{norm balance}},
\] so \(D = 0\) demands mutual orthogonality \textbf{and} equal column
norms --- that is orthoNORMality. \(C\) demands orthogonality alone, at
any column norms. Which is appropriate depends on the application: when
the components genuinely differ in magnitude, \(D\) charges a penalty
for the very structure the method exists to produce, and \(C\) does not.
In the data-anchored formulation of Section @ref(dataanchored) the two
coincide in practice, because the reconstruction term already forces the
column norms to agree.

The Euclidean gradient of the \(\tilde D\) form is available in closed
form, \[
...
\] requiring no factorisation. It satisfies the Euler identity
\(\langle \nabla D, Y\rangle = 0\), since \(D\) is homogeneous of degree
zero, so the orthogonality term cannot alter \(\|Y\|_F\); and
\(\|\nabla D\|_F \le 8 / \|Y\|_F\). For \(C\) the gradient is tangential
to each column's sphere separately, so \(C\) cannot alter \emph{any}
individual column norm.
...
A plain Euclidean gradient step,
\(Y \leftarrow Y - \eta \nabla_Y E(Y)\), does not respect the
constraints. The conventional remedy is Riemannian optimisation: project
the gradient onto the tangent space of the Stiefel manifold and apply a
\textbf{retraction} to return to it
(\citeproc{ref-absil2008optimization}{Absil, Mahony, and Sepulchre
2008}; \citeproc{ref-boumal2023intro}{Boumal 2023}). Retractions are
theoretically sound but require an SVD or an inverse matrix square root
at every step, and enforcing exact feasibility can fight the fidelity
term by abruptly correcting the iterate's geometry.

NSA-Flow takes the opposite view: keep non-negativity as a hard
constraint, which is cheap to enforce exactly, and treat orthogonality
as an \emph{objective} rather than a constraint. Nothing is retracted,
so no factorisation appears in the loop. Figure 1 illustrates how the
iterates evolve under different \(w\).

\subsubsection{Solver: spectral projected
gradient}\label{solver-spectral-projected-gradient}

The feasible set \(\{ Y \ge 0 \}\) is closed and convex and its
Euclidean projection is the elementwise clamp \(\Pi(Y) = \max(0, Y)\),
so the problem is a smooth objective on a convex set and admits a
projected-gradient method. We use the spectral projected gradient of
Birgin, Martínez and Raydan
(\citeproc{ref-birgin2000nonmonotone}{Birgin, Martı́nez, and Raydan
2000}): Barzilai--Borwein step lengths
(\citeproc{ref-barzilai1988two}{Barzilai and Borwein 1988}) \[
t_n = \frac{\langle s_n, s_n\rangle}{\langle s_n, r_n\rangle},
\qquad s_n = Y_n - Y_{n-1},\quad r_n = \nabla E(Y_n) - \nabla E(Y_{n-1}),
\] safeguarded by an Armijo backtracking line search on the projected
step \[
Y_{n+1} = \Pi\big(Y_n - t_n \nabla E(Y_n)\big),
\qquad
E(Y_{n+1}) \le E(Y_n) - \sigma \frac{\|Y_{n+1} - Y_n\|_F^2}{t_n}.
\] The line search makes the energy monotonically non-increasing by
construction, and for a continuously differentiable \(E\) on a closed
convex set the method converges to a stationary point.

\subsubsection{Stationarity certificate}\label{stationarity-certificate}

The quantity \[
\gamma_n = \frac{\|\Pi(Y_n - t_n \nabla E(Y_n)) - Y_n\|_F}{t_n}
\] is the norm of the projected-gradient mapping. It vanishes exactly at
a stationary point of the constrained problem, and unlike a gradient
norm it is valid \emph{on} the boundary, which is where non-negative
solutions live. We therefore report \(\gamma_n\) as a computable
certificate, and the solver reports which stopping rule fired --- the
certificate threshold, line-search exhaustion, or the iteration cap ---
rather than asserting convergence. Conditioning degrades as \(w \to 1\),
where the problem approaches its combinatorial limit: convergence takes
tens of iterations at \(w = 0.5\), hundreds at \(w = 0.9\), and
thousands beyond \(w = 0.99\).

A degeneracy is worth naming. At \(w = 1\) the fidelity term vanishes
and \(D\) is scale invariant, so the scale of the returned \(Y\) is
unconstrained; every scaled Stiefel matrix is optimal, and nothing
selects among clusterings either. The implementation warns at \(w = 1\)
and reports the observed scale drift.

\subsubsection{Why the penalty must be normalised per column, not
globally}\label{why-the-penalty-must-be-normalised-per-column-not-globally}

The unnormalised penalty \(\| Y^\top Y - I \|_F^2\) is sensitive to the
norm of \(Y\): scaling \(Y \to cY\) scales it by \(c^4\), which makes
the optimisation poorly conditioned and the step size hard to choose. An
earlier version of this work therefore used a globally normalised
off-diagonal defect, \[
L_{orth, inv}(Y) = \frac{\| Y^\top Y - \operatorname{diag}(\operatorname{diag}(Y^\top Y)) \|_F^2}{\|Y\|_F^4},
\] described as measuring the cosines of the angles between columns.
\textbf{It does not}, and the discrepancy has a concrete cost. Dividing
by the global \(\|Y\|_F^4\) rather than by \(\|y_i\|^2\|y_j\|^2\) leaves
a quantity that is invariant to a global rescaling but not to rescaling
the columns individually, so it is not a function of the angles alone.
More seriously, its zero set is too large: a matrix with one non-zero
column and \(k-1\) zero columns attains \(L_{orth, inv} = 0\) exactly,
because a zero column is orthogonal to everything. The functional is
therefore \emph{minimised by rank collapse}, and the same matrix is
scored \(0.2\) by \(C\) and \(0.8\) by \(D\).

The two penalties defined above fix this in different ways. \(C\)
normalises each column, so it is a genuine function of the angles and is
invariant to per-column rescaling, which is the real gauge freedom of a
basis; its diagonal charges a column whose norm has collapsed, which
doubles as a degeneracy penalty. \(D\) instead adds the norm-balance
term, and its floor \(D \ge 1/r - 1/k\) at rank \(r\) penalises collapse
directly. Either is a correct measure of what the method is asked to
produce; the off-diagonal defect alone is not, and it is not used here.
...
\item
  ...
  conjugate gradient methods. NSA-Flow instead avoids the manifold
  machinery altogether --- orthogonality is an objective, not a
  constraint --- which makes it a drop-in component for existing ML or
  deep-learning pipelines and lets the degree of constraint satisfaction
...
For tall-skinny matrices (\(p \gg k\)) each iteration costs one Gram
product and two \([p,k]\times[k,k]\) products, i.e.~\(O(pk^2)\), with no
SVD, eigendecomposition or QR anywhere in the loop --- the asymptotic
cost is the same as a retraction method's, but the constant is much
smaller and the per-step work is a few matrix multiplications rather
than a factorisation. When the basis is fitted directly to data with
\(p > n\) the Gram matrix is never formed either: every quantity follows
from \(XV\) and \(X^\top(XV)\), giving \(O(npk)\) per iteration and
removing an allocation that is \(0.4\) GB at \(p = 7129\) and \(3.2\) GB
at \(p = 20000\). Practically speaking, as will be shown in the Results
section, NSA-Flow provides an intuitive ``knob'' that allows users to
impose orthogonality, non-negativity and sparseness via a consistent and
intuitive framework. Table 1 summarizes these comparisons.
...
The NSA-Flow algorithm is implemented in \texttt{pytorch} as a small,
dependency-light package; the results in this document are produced by
calling it directly. The library is five modules and under a thousand
lines, a consolidation from the earlier version's eight modules and
roughly 2400 lines, achieved by deleting rather than adding.

The refinement entry point accepts a target \(X_0\), an optional
initialisation, and the weight \(w\); the estimation entry point accepts
the data matrix \(X\) and the number of components \(k\). What it no
longer accepts is the earlier version's configuration surface: there is
no retraction type, no optimizer backend, no learning-rate strategy, no
aggression or warm-up parameter, and no separate fidelity and
orthogonality type switches. Those existed to compensate for an energy
whose two terms were not comparably scaled, and they became unnecessary
once both terms were made dimensionless. The document records exactly
which arguments were dropped in Table @ref(tab:dropped).

Each iteration computes a fused value-and-gradient sharing a single Gram
product, takes a Barzilai--Borwein step, and accepts it through an
Armijo line search on the projected step; the energy is therefore
monotone by construction. Non-negativity is applied by exact projection.
Diagnostics recorded per iteration are the energy, the two loss terms,
the step length and the projected-gradient mapping norm, and the
returned object carries the stopping reason, the certificate value, the
iteration count, the effective rank and the scale ratio.

Two numerical choices are worth stating because both were errors in the
earlier version. The orthogonality defect is computed as
\(\|G - I/k\|_F^2\) directly rather than as \(\|G\|_F^2 - 1/k\); the
latter is algebraically identical but suffers catastrophic cancellation
near the optimum and returns small negative values. And when a polar
factor must be differentiated, the derivative is obtained by solving the
Sylvester equation, whose denominators are \(h_i + h_j > 0\), rather
than through the SVD backward pass, whose denominators are
\(\sigma_i^2 - \sigma_j^2\) and which returns \texttt{NaN} for
\emph{every} entry when singular values repeat --- as they do for any
orthogonal initialisation.

The implementation is research-grade but tested: the accompanying suite
asserts the propositions of this section executably, including the zero
set, the bounds, the spectral identity, the rank-collapse floor, the
invariances, the disjoint-support equivalence, agreement of every
closed-form gradient with automatic differentiation, and equality of the
Gram and matrix-free code paths. Experimentalists should still centre
and scale their inputs and choose \(w\) for the application; unlike the
earlier version, there is nothing else to tune.
...
We use default settings in the results below with \(w = 0.5\) unless
otherwise specified. Because both energy terms are dimensionless, there
is no relative weighting to set: \(w\) is the only balance parameter,
and it is not a nuisance parameter that has to be calibrated per
problem. There is likewise no learning rate to choose ---
Barzilai--Borwein steps are scale-free and the Armijo line search
accepts or halves them --- and convergence is declared on the
projected-gradient mapping norm falling below \(10^{-9}\) in double
precision (\(10^{-6}\) in single, which is above float32 machine
epsilon), or reported as unconverged otherwise. Empirically \(w = 0.5\)
is a better default than more aggressive settings: across the
experiments below it dominates \(w = 0.9\) on both reconstruction and
reproducibility, because large \(w\) pins the solution near a
combinatorial vertex where small perturbations of the data change which
vertex is selected.
...
{\centering \includegraphics[width=0.9\linewidth]{figs/nsa_flow_optimizer_comparison_wide_p100_k140} 
...
{\centering \includegraphics[width=0.9\linewidth]{figs/nsa_flow_optimization_paths_by_w} 
...
\subsection{Behaviour of the orthogonality weight across matrix
shapes}\label{behaviour-of-the-orthogonality-weight-across-matrix-shapes}

The earlier version of this work studied the strength of its retraction
operator, sweeping an interpolation parameter between a Euclidean step
and a polar retraction across tall, wide and square matrices. That
operator no longer exists --- orthogonality is now an objective and
nothing is retracted --- so the corresponding question is how the weight
\(w\) behaves across the same shapes, which is what we report below in
Section @ref(sparsity) and Figure 5.

For the record, the comparison against the retraction-based scheme is
not merely dropped. Run on the earlier study's own synthetic problem
(\(p = 200\), \(k = 8\)) and scored on its own metrics, the present
solver attains an orthogonality residual of \(0.011\) at \(w = 0.999\)
against \(0.016\) for polar and \(0.019\) for soft retraction, and
\(0.0018\) at \(w = 0.9999\); it matches the best retraction on that
study's energy (\(5.899\) against \(5.845\), with projected gradient at
\(59.5\)); it returns the identical basis from twelve random restarts
where polar retraction has a spread of \(9 \times 10^{-4}\); and its
runtime is nearly flat in \(p\) because no factorisation is performed.
The orthogonality residual is a \emph{dial} here rather than a fixed
property of the operator, which is the substantive difference between
the two formulations.

\subsection{\texorpdfstring{Sparsity as a Function of Orthogonality via
Weight Parameter
\(w\)}{Sparsity as a Function of Orthogonality via Weight Parameter w}}\label{sparsity}
...
NSA-Flow is then applied in its \emph{estimation} form: rather than
refining \(Y_0\), it fits a non-negative basis
\(V \in \mathbb{R}^{p\times k}\) to the centred data directly,
minimising
\((1-w)\lVert X - XVV^\top\rVert_F^2/\lVert X\rVert_F^2 + w\,C(V)\)
subject to \(V \ge 0\), with scores given by projection. We use \(Y_0\)
only as the PCA comparator. This matters: anchoring a non-negative
solution to a \emph{signed} target such as \(Y_0\) pulls the iterate
toward values it cannot reach, so the constrained optimum is close to a
clamp of the signed basis, which costs a factor \(1.36\) in
reconstruction error on these data against \(1.03\) for a basis fitted
to them. We sampled \(q\) random \(w\) values uniformly from {[}0.01,
0.99{]} to sample orthogonality configurations. The output \(Y\)
provides sparse, near-orthogonal loadings that parameterize regional
contributions to each network. Figure 10 shows the NSA-Flow fit to the
PCA loadings for a representative \(w = 0.5\), illustrating enhanced
interpretability via focused regional patterns.
...
The package exposes thirty public names. The ones needed for ordinary
use are few:

\begin{itemize}
\item
  \texttt{nsa\_flow.nsa\_flow(target,\ w,\ ...)}: refinement. Returns a
  non-negative, near-orthogonal matrix close to \texttt{target}. Reports
  \texttt{stop\_reason}, the certificate \texttt{grad\_map},
  \texttt{defect}, \texttt{effective\_rank} and \texttt{scale\_ratio}.
\item
  \texttt{nsa\_flow.nsa\_flow\_data(X,\ k,\ w,\ ...)}: estimation. Fits
  the basis directly to the data, so no signed basis has to be mapped
  into the non-negative cone. Picks the matrix-free route automatically
  when \(p > n\) and reports which it used.
\item
  \texttt{nsa\_flow.relax\_into\_nonneg(...)}: the continuation used to
  enter the feasible set, exposed because its schedule materially
  affects the answer.
\item
  \texttt{nsa\_flow.stiefel\_defect()},
  \texttt{nsa\_flow.angle\_defect()}: the orthoNORMality and
  orthogonality measures \(D\) and \(C\), with closed-form gradients
  \texttt{grad\_stiefel\_defect} and \texttt{grad\_angle\_defect}.
\item
  \texttt{nsa\_flow.project\_nonneg()},
  \texttt{nsa\_flow.project\_scaled\_stiefel()},
  \texttt{nsa\_flow.polar\_factor()}: exact Euclidean projections. Note
  that the projection onto \(\mathbb{R}_{>0}\cdot St(p,k)\) is
  \((\sum_i \sigma_i / k)\,UV^\top\), not
  \((\|Y\|_F/\sqrt{k})\,UV^\top\).
\item
  \texttt{nsa\_flow.NSAFlowLinear}, \texttt{NSAFlowConv2d},
  \texttt{NSAFlowLayer}: layers for use inside a network, with the
  per-sample structure of the input respected.
\item
  \texttt{nsa\_flow.nsa\_flow\_signed()}: experimental. Documented in
  the source with its own negative result; \texttt{nsa\_flow\_data} is
  preferred.
\end{itemize}

Functions present in the previous release and now absent, because the
machinery they configured is gone: \texttt{nsa\_flow\_orth},
\texttt{nsa\_flow\_retract\_auto},
\texttt{invariant\_orthogonality\_defect},
\texttt{get\_torch\_optimizer},
\texttt{estimate\_learning\_rate\_for\_nsa\_flow},
\texttt{get\_lr\_estimation\_strategies}.

A typical usage example. There is no optimizer, learning rate,
retraction or warm-up to specify:
...
The package ships a property-driven test suite, run with
\texttt{pytest\ tests/}. It is not a smoke test: the propositions of
Section 2 are asserted executably. Among them, that \(D\) vanishes
exactly on \(\mathbb{R}_{>0}\cdot St(p,k)\) and is bounded by
\(1 - 1/k\); that \(D = k\operatorname{Var}(\lambda)\) and
\(D \ge 1/r - 1/k\) at rank \(r\); that \(C\) is zero exactly on
mutually orthogonal columns at any norms, invariant to per-column
rescaling, and equal to one at collinearity; that for \(Y \ge 0\),
\(C = 0\) if and only if the column supports are pairwise disjoint,
checked against an independent combinatorial test over thousands of
random draws; that every closed-form gradient agrees with automatic
differentiation; that the Euler identities hold, globally for \(D\) and
per column for \(C\); that the Gram and matrix-free code paths return
the same optimum, up to the column permutation that is a genuine
symmetry of the problem; and that the solver reports \texttt{max\_iter}
rather than claiming convergence when it has not converged.

The suite also encodes the failure modes that motivated this version, so
they cannot silently return: that the off-diagonal-only defect scores
rank collapse at zero while \(C\) does not, and that a non-negative
basis fitted to the data reconstructs better than \texttt{abs()} of a
signed one.

The demonstration functions of the previous release
(\texttt{demo\_nsa\_flow\_optimizer\_tradeoff},
\texttt{demo\_nsa\_flow\_tradeoff}, \texttt{plot\_nsa\_trace}) compared
optimizer backends and learning-rate strategies, and have been removed
along with the machinery they compared.
...
\section{Mathematical Analysis: descent and
stationarity}\label{mathematical-analysis-descent-and-stationarity}

\textbf{Setup.} Let \(Y \in \mathbb{R}^{p\times k}\), let
\(\mathcal{C} = \{Y : Y \ge 0\}\), and let \[
E(Y) = (1-w)L_{fid}(Y, X_0) + w\,L_{orth}(Y)
\] be the energy of Section 2, with \(\Pi\) the Euclidean projection
onto \(\mathcal{C}\), which is the elementwise clamp
\(\Pi(Y) = \max(0,Y)\). The iteration is \[
Y_{n+1} = \Pi\big(Y_n - t_n \nabla E(Y_n)\big),
\] with \(t_n\) a Barzilai--Borwein step length safeguarded by Armijo
backtracking.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Smoothness.} \(E\) is continuously differentiable with locally
  Lipschitz gradient. This holds for \(L_{orth} = \tilde D\) wherever
  \(Y \neq 0\), and for \(L_{orth} = C\) wherever no column vanishes.
\item
  \textbf{Convex feasible set.} \(\mathcal{C}\) is closed and convex,
  and \(\Pi\) is its exact projection --- not an approximation, and
  requiring no factorisation.
\item
  \textbf{Bounded step lengths.} \(t_n \in [t_{\min}, t_{\max}]\) with
  \(0 < t_{\min} \le t_{\max} < \infty\).
\end{enumerate}

\subsection{Proposition (monotone
descent)}\label{proposition-monotone-descent}

The Armijo condition \[
E(Y_{n+1}) \le E(Y_n) - \sigma\,\frac{\|Y_{n+1} - Y_n\|_F^2}{t_n},\qquad \sigma > 0,
\] is satisfied for all sufficiently small \(t_n\), and the backtracking
loop halves \(t_n\) until it holds. Hence \(E(Y_n)\) is non-increasing,
and since \(E \ge 0\) it converges. \emph{Proof.} The projected-gradient
direction is a descent direction for a differentiable \(E\) on a convex
set, and the Armijo condition is satisfiable by smoothness; monotonicity
and boundedness below give convergence of the sequence of values.
\(\square\)

\subsection{Proposition (stationarity
certificate)}\label{proposition-stationarity-certificate}

Define the projected-gradient mapping norm \[
\gamma(Y, t) = \frac{\|\Pi(Y - t\nabla E(Y)) - Y\|_F}{t}.
\] Then \(\gamma(Y,t) = 0\) for some \(t > 0\) if and only if \(Y\)
satisfies the first-order condition for \(\min_{\mathcal{C}} E\), namely
\(\langle \nabla E(Y), Z - Y\rangle \ge 0\) for all
\(Z \in \mathcal{C}\). \emph{Proof.} \(\Pi(Y - t\nabla E(Y)) = Y\) is
precisely the fixed-point characterisation of the variational
inequality. \(\square\)

This is the reason \(\gamma\) is reported rather than \(\|\nabla E\|\):
at a constrained optimum on the boundary of \(\mathcal{C}\), which is
where non-negative solutions lie, the gradient need not vanish while
\(\gamma\) does.

\subsection{Remarks on what is and is not
claimed}\label{remarks-on-what-is-and-is-not-claimed}

The energy is not convex. In the refinement form the fidelity term is
convex and the orthogonality term is not; in the estimation form the
reconstruction term is quartic in \(V\). The propositions therefore give
monotone descent to a stationary point with a computable certificate,
and nothing about global optimality. In practice the relaxation path of
Section 2 is what makes the attained point reproducible: on the imaging
data of Section 4, refining the continuation schedule improves the
attained objective by roughly a factor of two, while increasing the
per-stage iteration budget twenty-fold changes it in the fourth decimal.
...
Barzilai, Jonathan, and Jonathan M. Borwein. 1988. {``Two-Point Step
Size Gradient Methods.''} \emph{IMA Journal of Numerical Analysis} 8
(1): 141--48. \url{https://doi.org/10.1093/imanum/8.1.141}.

\bibitem[\citeproctext]{ref-birgin2000nonmonotone}
Birgin, Ernesto G., José Mario Martı́nez, and Marcos Raydan. 2000.
{``Nonmonotone Spectral Projected Gradient Methods on Convex Sets.''}
\emph{SIAM Journal on Optimization} 10 (4): 1196--1211.
