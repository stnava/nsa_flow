...
To address these gaps, we propose **Non-negative Stiefel Approximating Flow (NSA-Flow)**, a variational optimization algorithm that approximates a target ( $X_0$ ) with a non-negative matrix ( $Y \in \mathbb{R}^{p \times k}_{\geq 0}$ ).  NSA-Flow balances fidelity, column orthogonality, and non-negativity through a single tunable parameter ( $w \in [0,1]$ ) influencing proximity to the Stiefel manifold.  The simple parameterization of these constraints means that NSA-Flow allows practitioners to directly encode the desired level of sparsity/decorrelation (which are closely related in this framework) in their embeddings, without the need for complex regularization schemes or full orthogonality constraints. As such, NSA-Flow employs global soft orthogonality constraints to promote disjoint support across columns and foster interpretable bases. Formulated to stay near the Stiefel manifold [@edelman1998geometry], it departs from Riemannian gradient descent [@absil2008optimization] in treating orthogonality as an objective rather than a constraint, so no retraction is required and no factorisation appears in the iteration. Non-negativity is the one hard constraint, enforced exactly by proximal projection [@parikh2014proximal], and descent is guaranteed by a line search rather than by manifold geometry. Conceptually, NSA-Flow functions as a soft projection operator that can be inserted into any existing machine learning system to improve interpretability—whether as a regularization layer in a neural network [@henaff2011deep; @ricci2025orthogonality], a refinement step in factor models [@li2023unilateral; @rahiche2022variational], or a sparsity-enforcing module in linear embeddings [@guo2019frobenius; @goessmann2020restricted]. Unlike purely regularization-based methods [@wu2023towards; @kurtz2023group; @ricci2025orthogonality], NSA-Flow may also operate as a one-sided projection operator, preserving input structure while enabling controlled decorrelation.
...
where \( w \in [0, 1] \) balances the fidelity loss \(L_{fid}\) against the orthogonality loss \(L_{orth}\). Both losses are *dimensionless by construction*, so no internal re-weighting, warm-up calibration or data-dependent scaling constant is required and \(w\) is a genuine convex weight. The fidelity term is the relative squared Frobenius distance
\[
L_{fid}(Y, X_0) = \frac{\| Y - X_0 \|_F^2}{\| X_0 \|_F^2},
\]
which is \(0\) at \(Y = X_0\) and exactly \(1\) at \(Y = 0\).

For the orthogonality loss, write \( G = Y^\top Y / \operatorname{tr}(Y^\top Y) \) for the trace-normalised Gram matrix. We use one of two penalties, and the distinction is not cosmetic:
\[
...
Crucially, \(D\) decomposes as
\[
D = \underbrace{\sum_{i \neq j} G_{ij}^2}_{\text{decorrelation}}
  + \underbrace{\sum_i \big(G_{ii} - \tfrac1k\big)^2}_{\text{norm balance}},
\]
so \(D = 0\) demands mutual orthogonality **and** equal column norms --- that is orthoNORMality. \(C\) demands orthogonality alone, at any column norms. Which is appropriate depends on the application: when the components genuinely differ in magnitude, \(D\) charges a penalty for the very structure the method exists to produce, and \(C\) does not. In the data-anchored formulation of Section \@ref(dataanchored) the two coincide in practice, because the reconstruction term already forces the column norms to agree.
...
NSA-Flow takes the opposite view: keep non-negativity as a hard constraint, which is cheap to enforce exactly, and treat orthogonality as an *objective* rather than a constraint. Nothing is retracted, so no factorisation appears in the loop. Figure 1 illustrates how the iterates evolve under different \(w\).
...
\]
is the norm of the projected-gradient mapping. It vanishes exactly at a stationary point of the constrained problem, and unlike a gradient norm it is valid *on* the boundary, which is where non-negative solutions live. We therefore report \(\gamma_n\) as a computable certificate, and the solver reports which stopping rule fired --- the certificate threshold, line-search exhaustion, or the iteration cap --- rather than asserting convergence. Conditioning degrades as \( w \to 1 \), where the problem approaches its combinatorial limit: convergence takes tens of iterations at \(w = 0.5\), hundreds at \(w = 0.9\), and thousands beyond \(w = 0.99\).

A degeneracy is worth naming. At \( w = 1 \) the fidelity term vanishes and \(D\) is scale invariant, so the scale of the returned \(Y\) is unconstrained; every scaled Stiefel matrix is optimal, and nothing selects among clusterings either. The implementation warns at \(w = 1\) and reports the observed scale drift.
...
\]
described as measuring the cosines of the angles between columns. **It does not**, and the discrepancy has a concrete cost. Dividing by the global \( \|Y\|_F^4 \) rather than by \( \|y_i\|^2\|y_j\|^2 \) leaves a quantity that is invariant to a global rescaling but not to rescaling the columns individually, so it is not a function of the angles alone. More seriously, its zero set is too large: a matrix with one non-zero column and \(k-1\) zero columns attains \( L_{orth, inv} = 0 \) exactly, because a zero column is orthogonal to everything. The functional is therefore *minimised by rank collapse*, and the same matrix is scored \(0.2\) by \(C\) and \(0.8\) by \(D\).

The two penalties defined above fix this in different ways. \(C\) normalises each column, so it is a genuine function of the angles and is invariant to per-column rescaling, which is the real gauge freedom of a basis; its diagonal charges a column whose norm has collapsed, which doubles as a degeneracy penalty. \(D\) instead adds the norm-balance term, and its floor \(D \ge 1/r - 1/k\) at rank \(r\) penalises collapse directly. Either is a correct measure of what the method is asked to produce; the off-diagonal defect alone is not, and it is not used here.
...
The NSA-Flow algorithm is implemented in `pytorch` as a small, dependency-light package; the results in this document are produced by calling it directly. The library is five modules and under a thousand lines, a consolidation from the earlier version's eight modules and roughly 2400 lines, achieved by deleting rather than adding.

The refinement entry point accepts a target \( X_0 \), an optional initialisation, and the weight \( w \); the estimation entry point accepts the data matrix \( X \) and the number of components \( k \). What it no longer accepts is the earlier version's configuration surface: there is no retraction type, no optimizer backend, no learning-rate strategy, no aggression or warm-up parameter, and no separate fidelity and orthogonality type switches. Those existed to compensate for an energy whose two terms were not comparably scaled, and they became unnecessary once both terms were made dimensionless. The document records exactly which arguments were dropped in Table \@ref(tab:dropped).

Each iteration computes a fused value-and-gradient sharing a single Gram product, takes a Barzilai--Borwein step, and accepts it through an Armijo line search on the projected step; the energy is therefore monotone by construction. Non-negativity is applied by exact projection. Diagnostics recorded per iteration are the energy, the two loss terms, the step length and the projected-gradient mapping norm, and the returned object carries the stopping reason, the certificate value, the iteration count, the effective rank and the scale ratio.

Two numerical choices are worth stating because both were errors in the earlier version. The orthogonality defect is computed as \( \|G - I/k\|_F^2 \) directly rather than as \( \|G\|_F^2 - 1/k \); the latter is algebraically identical but suffers catastrophic cancellation near the optimum and returns small negative values. And when a polar factor must be differentiated, the derivative is obtained by solving the Sylvester equation, whose denominators are \( h_i + h_j > 0 \), rather than through the SVD backward pass, whose denominators are \( \sigma_i^2 - \sigma_j^2 \) and which returns `NaN` for *every* entry when singular values repeat --- as they do for any orthogonal initialisation.

The implementation is research-grade but tested: the accompanying suite asserts the propositions of this section executably, including the zero set, the bounds, the spectral identity, the rank-collapse floor, the invariances, the disjoint-support equivalence, agreement of every closed-form gradient with automatic differentiation, and equality of the Gram and matrix-free code paths. Experimentalists should still centre and scale their inputs and choose \( w \) for the application; unlike the earlier version, there is nothing else to tune.
...
We use default settings in the results below with \( w = 0.5 \) unless otherwise specified. Because both energy terms are dimensionless, there is no relative weighting to set: \( w \) is the only balance parameter, and it is not a nuisance parameter that has to be calibrated per problem. There is likewise no learning rate to choose --- Barzilai--Borwein steps are scale-free and the Armijo line search accepts or halves them --- and convergence is declared on the projected-gradient mapping norm falling below \( 10^{-9} \) in double precision (\( 10^{-6} \) in single, which is above float32 machine epsilon), or reported as unconverged otherwise. Empirically \( w = 0.5 \) is a better default than more aggressive settings: across the experiments below it dominates \( w = 0.9 \) on both reconstruction and reproducibility, because large \( w \) pins the solution near a combinatorial vertex where small perturbations of the data change which vertex is selected.

## Toy Example: Decomposing a Small Mixed-Signal Matrix

To intuitively illustrate NSA-Flow, consider a toy 4x3 matrix \( X_0 \) representing mixed signals: each column is a nonnegative orthogonal basis vector (e.g., distinct patterns), but observed with noise and scaling. NSA-Flow approximates an orthogonal nonnegative basis \( Y \) close to \( X_0 \).  Figure 3 shows the results of applying NSA-Flow to this toy example, starting from a random initialization \( Y_0 \).
...
A key choice in NSA-Flow is the optimization algorithm. We compare several optimizers: standard gradient descent, Adam, Armijo gradient descent, AdaGrad and others. Each has different convergence properties and sensitivities to hyperparameters and we evaluate them with standard defaults and an automatically estimated learning rate via Armijo condition [@armijo1966minimization]. We evaluate them on synthetic data measuring convergence speed and final orthogonality/fidelity metrics.  Performance across $\omega$ values are shown in Figure 4.  A data-driven ranking of the methods based on execution time and objective values shows that `pytorch` averaged stochastic gradient descent [@polyak1992acceleration] and Layer-wise Adaptive Rate Scaling (LARS) [@you2017large] perform well in terms of both speed and objective values across a variety of problems.

```{r pollen3, echo = FALSE, out.width='90%', fig.cap = "The default optimizer peformance across different values of orthogonality weight indicates that the single parameter provides predictable control of the orthogonality in the outcome.  Higher values lead predictably to more decorrelation at the expense of fidelity."}
knitr::include_graphics("figs/nsa_flow_optimization_paths_by_w.png")
```

## Behaviour of the orthogonality weight across matrix shapes

The earlier version of this work studied the strength of its retraction operator, sweeping an interpolation parameter between a Euclidean step and a polar retraction across tall, wide and square matrices. That operator no longer exists --- orthogonality is now an objective and nothing is retracted --- so the corresponding question is how the weight \( w \) behaves across the same shapes, which is what we report below in Section \@ref(sparsity) and Figure 5.

For the record, the comparison against the retraction-based scheme is not merely dropped. Run on the earlier study's own synthetic problem (\( p = 200 \), \( k = 8 \)) and scored on its own metrics, the present solver attains an orthogonality residual of \( 0.011 \) at \( w = 0.999 \) against \( 0.016 \) for polar and \( 0.019 \) for soft retraction, and \( 0.0018 \) at \( w = 0.9999 \); it matches the best retraction on that study's energy (\( 5.899 \) against \( 5.845 \), with projected gradient at \( 59.5 \)); it returns the identical basis from twelve random restarts where polar retraction has a spread of \( 9 \times 10^{-4} \); and its runtime is nearly flat in \( p \) because no factorisation is performed. The orthogonality residual is a *dial* here rather than a fixed property of the operator, which is the substantive difference between the two formulations.
...
```{r sparse_pca_soft_B, echo=FALSE,eval=FALSE}
...
```{r golub_sparse_pca_analysis, echo=FALSE, fig.width=5,  message=FALSE, warning=FALSE,cache=FALSE,eval=!exists("golub_metrics")}
...
```{r pca_sens_metrics, echo=FALSE, fig.width=7, fig.height=4, message=FALSE, warning=FALSE,eval=FALSE,cache=FALSE}
# Sensitivity Sweep (Ω)
...
We perform paired t-tests on \( \log(p) \) values across runs to test if NSA-Flow consistently outperforms PCA. Across a broad range of cognitive and functional outcomes, the NSA-Flow model shows consistent advantages over PCA for most variables, with a few exceptions. Specifically:

* NSA-Flow outperforms PCA on 5 out of 9 cognitive outcomes on average (CDRSB, ADAS13, ADASQ4, mPACCdigit, EcogSPTotal).

* PCA performs slightly better on FAQ, MMSE and EcogPtTotal which emphasize global cognition and where linear variance-based projections tend to align well with simple latent factor structure.

* The average log-likelihood improvement (Δlog p) for NSA over PCA is between −1.0 and −6.0 units for the majority of outcomes, which corresponds to a meaningful difference in predictive fit under the cross-validation framework.

* The improvement differences (bottom panel Figure 13) vary smoothly with \( w \), indicating well-behaved optimization.

Boxplots in Figure 13 show \( \log(p) \) distributions by method and cognitive variable. Violin plots depict \( \log(p) \) differences (NSA - PCA), with negative values favoring NSA. A scatter plot relates sparsity (tuned by w) to performance gains (Figure 13 bottom panel).  These vary smoothly with w, indicating well-behaved optimization.

### Extension to Diagnosis Prediction

To further rigorously assess NSA-Flow's advantages over PCA, we incorporate a trinary classification task for AD diagnosis between controls (CN) vs. mild cognitive impairment (MCI) vs clinical Alzheimer's disease (AD, most severe symptoms) derived from ADNI diagnostic labels using random forest classification and ROC analysis [@Cuingnet2011]. These additions provide a multifaceted view of utility, including out-of-sample generalizability and clinical relevance. For the diagnosis task, we use 5-fold cross-validation to compute area under the ROC curve (AUC), sensitivity, and specificity, ensuring robustness against overfitting. Table 3 summarizes these results.  NSA-Flow yields an ~4.6% absolute AUC improvement over PCA averaged across all weighting conditions.  Figure 11 demonstrates stability of this effect across different $w$ choices.  While this difference may appear small numerically, it is: 
...
NSA-flow also demonstrates meaningful variable selection.  The regions that are highlighted by the two most predictive components (4th and 5th shown in Figure 12) are consistent with known AD pathology, including frontal, medial temporal lobe structures and cholinergic regions (basal forebrain). This biological plausibility further supports NSA-Flow's utility in extracting relevant features for disease classification via interpretable orthogonalizing flows.
...
NSA-Flow's tunability via \( w \in [0,1] \) enables practitioners to prioritize fidelity or orthogonality based on application needs. For instance, low \( w \) (0.05–0.25) yields moderate orthogonality defect reduction with relatively low fidelity loss in synthetic tests, ideal for clustering tasks requiring minimal decorrelation [@ding2006orthogonal]. Higher \( w \) values suit applications like sparse PCA, where orthogonality enhances feature independence [@yoo2009orthogonal]. The `python` package implementation is modular, with helper functions for stable matrix operations and diagnostics for monitoring convergence [@absil2008optimization]. Backtracking line search ensures robustness to step-size selection [@parikh2014proximal], while adaptive learning rates enhance efficiency. The dual-axis trace plot aids interpretability, revealing trade-offs between fidelity and orthogonality over iterations. Practitioners should calibrate \( w \) via cross-validation, as optimal values depend on data sparsity and noise levels [@strazar2016orthogonal].  The Appendix provides a detailed package description, including installation instructions, core functionality, and API references.

## Empirical Performance

Empirical results highlight NSA-Flow's potential benefits. On the Golub leukemia dataset, NSA-Flow improves classification accuracy by 6% over PCA and and 2% over (standard) sparse PCA, identifying interpretable biomarkers due to its non-negative, semi-orthogonal embeddings [@strazar2016orthogonal].  Qualitatively, the embeddings (projections) also appear to be more separable in this application.  In Alzheimer's disease, NSA-Flow-derived networks yield better cognitive outcome predictions and diagnosis classification (AUC improvements of ~4.6% over PCA), with loadings aligning with known pathology (e.g., medial temporal lobe involvement) [@Chen2024]. These findings suggest that NSA-Flow effectively balances interpretability and predictive power, making it a valuable tool for biomedical data analysis.
...
The package is compatible with PyTorch for seamless integration into deep learning workflows and supports joint optimization tasks. It emphasizes simplicity of use and differentiability, making it suitable for applications where interpretability and constraint enforcement are critical.

## Key Features

- **Manifold Approximation**: Implements a smooth geometric flow near the Stiefel manifold to handle orthogonality constraints continuously.

- **Constraint Enforcement**: Incorporates non-negativity via proximal operators and orthogonality through retraction methods.

- **Optimization Flexibility**: Supports various PyTorch optimizers (e.g., ASGD) with adaptive learning rates and strategies like Bayesian optimization for hyperparameter tuning.

- **Metrics and Visualization**: Includes functions for computing orthogonality defects (e.g., invariant_orthogonality_defect and defect_fast) and plotting optimization traces.
...
The energy is not convex. In the refinement form the fidelity term is convex and the orthogonality term is not; in the estimation form the reconstruction term is quartic in \( V \). The propositions therefore give monotone descent to a stationary point with a computable certificate, and nothing about global optimality. In practice the relaxation path of Section 2 is what makes the attained point reproducible: on the imaging data of Section 4, refining the continuation schedule improves the attained objective by roughly a factor of two, while increasing the per-stage iteration budget twenty-fold changes it in the fourth decimal.
