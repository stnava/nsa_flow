...
def nsa_flow_retract_auto(
    ...
    try:
        ...
        if retraction_type in ["soft_polar", "polar"]:
            # Differentiable SVD (using float64 internally for stability)
            orig_dtype = Y.dtype
            ...
            # Column-wise normalization (dim -2 is rows)
...
# nsa_flow is now an alias for nsa_flow_orth (the modern autograd-safe version)
def nsa_flow(*args, **kwargs):
    return nsa_flow_orth(*args, **kwargs)

# Alias for backward compatibility with tests and older code
...
class NSAFlowLayer(nn.Module):
    ...
    def __init__(
        ...
        # Learnable retraction weight (stored as logit for sigmoid constraint)
    ...
    def forward(self, Y, target=None):
        ...
        # Get the pure orthogonal projection of the proposal
        Y_orth = nsa_flow_retract_auto(Y_prop, w_retract=1.0, retraction_type=self.retraction_type)

        if self.residual:
            # Convex combination blend (The "Flow" toward the manifold)
...
class NSAFlowLinear(nn.Module):
    ...
    This replaces standard nn.Linear(in_features, out_features) while 
    offering better feature decorrelation and stability.
    """
    def __init__(
        self,
        in_features: int,
        ...
        # The underlying unconstrained weight matrix W_raw [in_features, out_features]
...
class NSAFlowConv2d(nn.Conv2d):
    ...
    towards the Stiefel manifold. This enforces orthogonal or semi-orthogonal 
    filter banks, improving Lipschitz continuity and training stability in deep CNNs.
    """
    def __init__(
        self,
        in_channels: int,
    ...
    def get_manifold_weight(self):
        ...
        # To orthogonalize the filters, we flatten the spatial and input channel dimensions
        # self.weight is [out_channels, in_channels/groups, kH, kW]
        # We transpose to [fan_in, out_channels] so that the out_channels are orthogonal to each other.
